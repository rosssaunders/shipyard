                ...
                if is_reading_line(trimmed) {
                        ...
                        && !parser.files_read.contains(&file)
                    {
                        ...
                        if parser.current_stage != Stage::Reading {
                            ...
                            add_event(&state, &task_id, "stage", "📖", "Reading codebase...", None);
                        }
                        // Batch file reads — emit every 3 files
                        if parser.files_read.len().is_multiple_of(3) {
                            let recent: Vec<_> = parser.files_read.iter().rev().take(3).collect();
                            add_event(
                                &state,
                                &task_id,
                                "stage",
                    ...
                    if parser.current_stage != Stage::Thinking {
                        ...
                        add_event(
                            ...
                            "stage",
                            "🤔",
                            &format!("Thinking: {}", truncate(&thought, 100)),
                            None,
                        );
                    }
                    ...
                    add_event(
                        ...
                        "stage",
                        "📝",
                        &format!("Committing: {}", truncate(trimmed, 80)),
                        None,
                    );
                    ...
                    add_event(
                        ...
                        "stage",
                        "🧪",
                        &format!("Test result: {}", truncate(trimmed, 100)),
                        None,
                    );
                    ...
                    add_event(
                        ...
                        "stage",
                        "⚠️",
                        &format!("Error: {}", truncate(trimmed, 100)),
                        None,
                    );
                }
            }
